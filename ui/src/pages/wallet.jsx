...
function Wallet({ clientConfig, setClientConfig }) {
  ...
  const [smartWalletSBtc, setSmartWalletSBtc] = useState({});
  ...
  async function initWalletbalance() {
    ...
    const {
      ...
      nonFungibleTokens: user_nonFungibleTokens,
    } = await getUserBalance(clientConfig);

    console.log({
      smartwallet_stx,
      smartwallet_fungibleTokens,
      smartwallet_nonFungibleTokens,
      user_stx,
      user_fungibleTokens,
      user_nonFungibleTokens,
    });
  ...
  async function initWalletInstance() {
    ...
    if (!contract_info?.found) {
      setAdvisoryMessage({
        msg: 'Seems you dont have smart wallet contract deployed yet.',
        reason: 'Deploy Required',
        severity: 'secondary',
      });
      setShowSmartWallettModal(true);
    }
    if (contract_info?.error) {
      setAdvisoryMessage({
        msg: contract_info?.error,
        reason: contract_info?.code,
        severity: 'danger',
      });
    }
  }
  ...
  return (
    <>
      <div className="w-full flex flex-col justify-center gap-6 items-center mt-10 p-2 sm:p-[5%] md:p-[10%] lg:p-[10%]">
        ...
        <SmartWalletBalance
          balance={formatNumber(parseFloat(smartWalletStx?.balance) / 1000000)}
          stx={smartWalletStx}
          setShowDepositModal={setShowDepositModal}
          setShowStxSendModal={setShowStxSendModal}
        />

        <Tabs
          clientConfig={clientConfig}
          fungibleToken={smartWalletFungibleToken}
          nonFungibleToken={smartWalletNonFungibleToken}
      ...
      {showDepositModal && (
        <DepositModal
          ...
          stx={userStx}
          fungibleToken={userFungibleToken}
          nonFungibleToken={userNonFungibleToken}
          clientConfig={clientConfig}
          setTx={setTx}
          setConfirmationModal={setConfirmationModal}
          contractState={contractState}
      ...
      {showSmartWalletModal && (
        <SmartWalletDeployModal
          show={showSmartWalletModal}
          close={() => setShowSmartWallettModal(false)}
          clientConfig={clientConfig}
          setTx={setTx}
          setConfirmationModal={setConfirmationModal}
          contractState={contractState}
      ...
      {showStxSendModal && (
        <StxSendModal
          show={showStxSendModal}
          close={() => setShowStxSendModal(false)}
          stx={smartWalletStx}
          clientConfig={clientConfig}
          setTx={setTx}
          setConfirmationModal={setConfirmationModal}
